...
def parse_args():
    ...
    parser.add_argument(
        '--config', default='src/configs/task/robustness/r4-adversarial-untarget.yaml', help='config file path')
    parser.add_argument(
        '--cfg-options',
        nargs='+',
        action=DictAction,
...
if __name__ == '__main__':
    ...
    with open(config, 'r') as f:
        ...
        runner = BaseTask(dataset_id=dataset_id, model_id=model_id, method_cfg=method_cfg, dataset_cfg=dataset_cfg,
                          generation_kwargs=generation_kwargs, log_file=log_file, evaluator_seq_cfgs=evaluator_seq_cfgs)
        runner.pipeline()
