...
class VizWiz(BaseDataset):
    dataset_ids: Sequence[str] = ["vizwiz-recognition",
                                  "vizwiz-recognition-pri-query", "vizwiz-drawline-recognition-pri-query"]
    dataset_config: Optional[Dict[str, str]] = {
        "vizwiz-recognition": "src/configs/datasets/vizwiz-recognition.yaml",
        "vizwiz-recognition-pri-query": "src/configs/datasets/vizwiz-recognition-pri-query.yaml",
        "vizwiz-drawline-recognition-pri-query": "src/configs/datasets/vizwiz-drawline-recognition-pri-query.yaml",
    }
    ...
    def template_format(self, anno: Dict) -> ImageTxtSample:
        if self.dataset_id == "vizwiz-recognition":
            ...
            image_path = os.path.join(self.image_dir, anno['image'])
            text = self.recoginiton_template_pri_query_drawline.format(
                question=anno['question'])
            target = anno["private_qa"]
        else:
            raise ValueError
