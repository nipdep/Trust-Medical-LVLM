...
class Vispr(BaseDataset):
    ...
    dataset_ids: Sequence[str] = ["vispr-recognition", "vispr-leakage",
                                  "vispr-leakage-protected", "vispr-recognition-pri-query"]
    dataset_config: Optional[Dict[str, str]] = {
        "vispr-recognition": "src/configs/datasets/vispr-recognition.yaml",
        "vispr-recognition-pri-query": "src/configs/datasets/vispr-recognition-pri-query.yaml",
        "vispr-leakage": "src/configs/datasets/vispr-leakage.yaml",
        "vispr-leakage-protected": "src/configs/datasets/vispr-leakage.yaml",
    }
    ...
    def template_format(self, anno: Dict) -> ImageTxtSample:
        if self.dataset_id == "vispr-recognition":
            ...
            image_path = os.path.join(self.image_dir, anno['image'])
            text = self.recoginiton_template_pri_query.format(
                question=anno['question'])
            target = anno["private_qa"]

        elif self.dataset_id == "vispr-leakage":
            image_path = os.path.join(
                self.image_dir, anno['SheetName'], anno['Img Path'])
            ...
            image_path = os.path.join(
                self.image_dir, anno['SheetName'], anno['Img Path'])
            text = self.leakage_protected_template.format(
                type=anno["Info Type"])
            target = anno["Content"]
        else:
            raise ValueError
    ...
    def __init__(self, dataset_id: str, method_hook: Optional[BaseMethod] = None, **kwargs) -> None:
        ...
        if self.annotation_file.endswith('.jsonl') or "vispr-leakage" in self.dataset_id:
            annotations = [json.loads(q)
                           for q in open(self.annotation_file, "r")]
        else:
