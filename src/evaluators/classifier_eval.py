...
class ClassiferActionEvaluator(BaseEvaluator):
    ...
    def __init__(self, evaluator_id: str, metrics_cfg: Dict[str, Any], label_fields: Optional[List[str]] = None) -> None:
        ...
        # ro means response-only model
        self.classifier = pipeline(
            model='LibrAI/longformer-action-ro', device="cuda")
    ...
    def process(self, preds: Sequence[Any], labels: Sequence[Any], extras: Sequence[Any], **kwargs) -> Tuple[Sequence[Any], Sequence[Any]]:
        ...
        for pred in preds:
            ...
            if classification[0]["label"] in self.label_fields:
...
class HarmBenchModelEvaluator(BaseEvaluator):
    ...
    def __init__(self, evaluator_id: str, metrics_cfg: Dict[str, Any], device: str = "cuda") -> None:
        ...
        self.cls = AutoModelForCausalLM.from_pretrained(
            model_path, torch_dtype=torch.bfloat16).to(device)  # device_map=auto
    ...
    def process(self, preds: Sequence[Any], labels: Sequence[Any], extras: Sequence[Any], **kwargs) -> Tuple[Sequence[Any], Sequence[Any]]:
        ...
        for (pred, label) in zip(preds, labels):
            inputs = [LLAMA2_CLS_PROMPT['prompt'].format(
                behavior=label, generation=pred)]
